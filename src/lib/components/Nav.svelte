...
<script>
	...
	import { observing, colors } from '$lib/stores.js';
	export let segment;
	let isScrolling;
	let isIdle = true;
...
<nav class="navbar {$observing ? "is-transparent" : "iswhite"}" role="navigation" aria-label="main navigation" in:fade={{duration:1000, delay:50, easing: quadInOut}} out:fade={{duration:400, delay:0, easing: quadInOut}} bind:this={navbar} >
	...
	<div class="navbar-end is-hidden-touch">
		...
		<a bind:this={navItems[1]} on:mouseenter={()=>handleIndicator(1)} class="navbar-item" aria-current={segment === 'produits' ? 'page' : undefined} href="/produits" on:click={() => menuIsActive = false}>Produits</a>
	...
	<div class="navbar-menu" class:is-active={menuIsActive} transition:slide={{easing: quadInOut}} >
	  <div class="navbar-start">
		<a class="navbar-item" aria-current={segment === 'produits' ? 'page' : undefined} href="/produits" on:click={() => menuIsActive = false}>Produits</a>
